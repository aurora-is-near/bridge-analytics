...
async function main() {
  ...
  while (true) {
    while (true) {
      try {
        ...
        if (ERC_TOKEN_DEPOSIT) {
          storeData(
            ERC_TOKEN_DEPOSIT,
            path.join(__dirname, 'assetHistory', `deposit_${TIME_THRESHOLD}`)
          );
        }
        if (ERC_TOKEN_WITHDRAW) {
          storeData(
            ERC_TOKEN_WITHDRAW,
            path.join(__dirname, 'assetHistory', `withdraw_${TIME_THRESHOLD}`)
          );
        }
        if (ERC_TOKEN_ASSET) {
          storeData(
            ERC_TOKEN_ASSET,
            path.join(__dirname, 'assetHistory', `asset_${TIME_THRESHOLD}`)
          );
        }
        ...
        console.log(
          'current time: ' +
            moment(TIME_THRESHOLD).format('YYYY-MM-DD HH:mm:ss')
        );
        break;
      } catch (e) {
    ...
    while (
      ERC_TOKEN_ASSET &&
      fs.existsSync(`./assetHistory/asset_${TIME_THRESHOLD}`)
    ) {
      try {
    ...
    while (
      ERC_TOKEN_DEPOSIT &&
      fs.existsSync(`./assetHistory/deposit_${TIME_THRESHOLD}`)
    ) {
      try {
        loadJsonToBigquery(
          `./assetHistory/deposit_${TIME_THRESHOLD}`,
          'deposit'
        );
        break;
      } catch (e) {
    ...
    while (
      ERC_TOKEN_WITHDRAW &&
      fs.existsSync(`./assetHistory/withdraw_${TIME_THRESHOLD}`)
    ) {
      try {
        loadJsonToBigquery(
          `./assetHistory/withdraw_${TIME_THRESHOLD}`,
          'withdrawl'
        );
        break;
      } catch (e) {
...
async function getERCtokenAsset() {
  ...
  let response = await tFetch(
    `https://api.etherscan.io/api?module=account&action=tokentx&address=${ETH_ADDRESS}&startblock=0&endblock=999999999&sort=asc&apikey=${API_KEY}`,
    {
      headers: {
  ...
  ERC_TOKEN_ASSET = ERCtokenAsset
    ? ERCtokenAsset.map(JSON.stringify).join('\n')
    : null;
  ERC_TOKEN_DEPOSIT = ERCtokenDeposit
    ? ERCtokenDeposit.map(JSON.stringify).join('\n')
    : null;
  ERC_TOKEN_WITHDRAW = ERCtokenWithdrawl
    ? ERCtokenWithdrawl.map(JSON.stringify).join('\n')
    : null;
}
...
const aggregateTokenMap = (array) => {
  for (let i = 0; i < array.length; i++) {
    if (!ERCtokenList.get(array[i].tokenSymbol)) {
      ERCtokenList.set(array[i].tokenSymbol, {
        symbol: array[i].tokenSymbol,
        address: array[i].contractAddress,
        decimals: array[i].tokenDecimal,
      });
    }
  }
};
